...
export const bulkRecipeProcessor = async (
  recipes: RecipeIn[],
): Promise<{ totalAdded: number; rejected: RecipeIn[] }> => {
  const currentRecipes = await recipesApi.getAll();
  let totalAdded = 0;
  const rejected: RecipeIn[] = [];

  for (const recipe of recipes) {
    let dbRecipe = currentRecipes.find(
      ...
      for (const ingredient of recipe.ingredients) {
        ...
        if (!existingIngredient) {
          ...
          try {
            ...
            console.error(
              `Failed to add ingredient ${ingredient.name} to recipe ${dbRecipe.name}:`,
              error,
            );
