...
export const FileUploader = ({
  ...
  useEffect(() => {
    if (file) {
      void parseContents(file).then((recipes) => {
        setContent(recipes);
      });
    }
  }, [file]);
    ...
    <DialogTrigger>
      ...
      <Modal isDismissable>
        <Dialog className="p-5">
          <FileTrigger
            acceptedFileTypes={["application/json", "text/csv", ".csv"]}
            onSelect={(e) => {
              const files = e ? Array.from(e) : [];
              setFile(files[0]);
            }}
          >
            <Button className="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-blue-700">
              Select file
            </Button>
          </FileTrigger>
